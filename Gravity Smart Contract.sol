...
contract Gravity {
    ...
    function updateSupply() internal returns (uint256) {
      ...
      if (now - timeOfLastHalving >= 2628000 minutes) {
        reward /= 2;
        timeOfLastHalving = now;
      }

      if (now - timeOfLastIncrease >= 300 seconds) {
        uint256 increaseAmount = ((now - timeOfLastIncrease) / 300 seconds) * reward;
